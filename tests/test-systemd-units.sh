...
write_third_party_unit "$third_party_unit" /usr/local/bin/codex \
  /etc/codex-remote-provider/provider.env stable_session gpt-5.6-sol high
write_official_unit "$official_unit" /usr/local/bin/codex stable_session
...
grep -Fxq 'ExecStart=/usr/local/bin/codex remote-control start --json -c model_provider=stable_session -c model=gpt-5.6-sol -c model_reasoning_effort=high' "$third_party_unit"
if grep -Fq 'EnvironmentFile=' "$official_unit"; then
  printf 'official unit unexpectedly loads the third-party secret\n' >&2
  exit 1
fi
grep -Fxq 'ExecStart=/usr/local/bin/codex remote-control start --json -c model_provider=stable_session' "$official_unit"
