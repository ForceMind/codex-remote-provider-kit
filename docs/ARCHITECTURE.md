...
macOS 另外把配置状态分为官方、本工具第三方和外部管理三类。安装只允许从官方状态
开始；后续检测到 CC Switch 等工具选择了外部 provider 时，写操作会停止而不是争抢
顶层默认值。macOS 回滚按字段和带标记区块撤销本工具的修改，保留安装后新增的其他
provider；从官方切换第三方前会刷新最近官方默认值快照。同名 provider 或专用
profile 发生外部修改时拒绝猜测所有权。Windows
目前仍使用安装前完整配置备份恢复，不应把这一 macOS 共存保证外推到 Windows。
...
目录。更新通过 HTTPS 下载 `main` 安装器和归档，先校验安装器语法及归档入口，
再复用在线安装器的备份/替换事务。失败时启动器继续执行当前版本，更新过程不会
读取密钥、修改 Codex 配置、切换 provider 或重启 Remote。
